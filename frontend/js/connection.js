...
// ★ target 역계산 (fallback): TP/SL 거리 → target 금액
function calcTargetFromTPSL(entry, tp, sl, volume, symbol) {
    if (!entry || entry <= 0 || !tp || tp <= 0 || !sl || sl <= 0) return 0;
    const specs = (typeof SYMBOL_SPECS !== 'undefined') ? SYMBOL_SPECS[symbol] : null;
    if (!specs) return 0;
    const tickSize = specs.tick_size || 0.01;
    const tickValue = specs.tick_value || 0.01;
    const ppp = volume * tickValue / tickSize;
    if (ppp <= 0) return 0;
    const tpDist = Math.abs(tp - entry);
    return Math.round(tpDist * ppp);
}
// ★ localStorage에서 target 복원
function qeLoadTargetLS(symbol) {
    try {
        const all = JSON.parse(localStorage.getItem('qe_positions') || '{}');
        return (all[symbol] && all[symbol].target > 0) ? all[symbol].target : 0;
    } catch(e) { return 0; }
}
...
document.addEventListener('visibilitychange', function() {
    if (document.hidden) {
        ...
        // 60초 이상 백그라운드 — WS 재연결 + softRefresh
        if (_bgDuration > 60000) {
            console.log('[Visibility] 🔄 60초 이상 백그라운드 — WS 재연결 + softRefresh');
            reconnectAttempt = 0; if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; } connectWebSocket(); setTimeout(function() { _lastSoftRefreshAt = 0; softRefresh('bg_return'); }, 1000);
            return;
        }

        // WS가 끊어져 있으면 재연결 + 서버 체크
        if (!ws || (ws.readyState !== WebSocket.OPEN && ws.readyState !== WebSocket.CONNECTING)) {
            console.log('[Visibility] WS 재연결 필요 — 서버 체크');
            reconnectAttempt = 0;
            if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; }
            // ★★★ 모바일: 서버 체크 (오버레이 없이 조용히 재연결) ★★★
            checkServerHealth().then(alive => {
                if (alive) {
                    hideReconnectOverlay();
                    stopServerHealthPoll();
                    console.log('[Visibility] 서버 정상 — 조용히 재연결');
                } else {
                    // 서버 다운이면 10초 후에만 오버레이 (바로 표시 안 함)
                    if (!_reconnectOverlayTimer) {
                        _reconnectOverlayTimer = setTimeout(async () => {
                            const stillDown = !(await checkServerHealth());
                            if (stillDown && (!ws || ws.readyState !== WebSocket.OPEN)) {
                                showReconnectOverlay();
                                startServerHealthPoll();
                            }
                        }, 10000);
                    }
                }
...
// ★★★ 모바일 서버복구 자동 감지 시스템 ★★★
let _serverHealthTimer = null;
let _serverIsDown = false;
let _reconnectOverlayTimer = null;

async function checkServerHealth() {
    try {
        const controller = new AbortController();
        setTimeout(() => controller.abort(), 5000);
        const r = await fetch(API_URL + '/health', { method: 'GET', cache: 'no-cache', signal: controller.signal });
        return r.ok;
    } catch (e) { return false; }
}

function showReconnectOverlay() {
    let ov = document.getElementById('reconnectOverlay');
    if (!ov) {
        ov = document.createElement('div');
        ov.id = 'reconnectOverlay';
        ov.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.85);z-index:99999;display:none;flex-direction:column;align-items:center;justify-content:center;gap:16px;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);';
        ov.innerHTML = '<div style="width:48px;height:48px;border:3px solid rgba(0,212,255,0.3);border-top:3px solid #00d4ff;border-radius:50%;animation:reconnSpin 1s linear infinite;"></div><div id="reconnMsg" style="color:#fff;font-size:16px;font-weight:600;text-align:center;">서버에 연결하는 중...</div><div id="reconnSub" style="color:rgba(255,255,255,0.5);font-size:13px;text-align:center;">자동으로 재연결됩니다</div><button onclick="location.reload()" style="margin-top:12px;padding:10px 28px;background:rgba(0,212,255,0.15);border:1px solid rgba(0,212,255,0.4);color:#00d4ff;border-radius:8px;font-size:14px;cursor:pointer;">수동 새로고침</button>';
        const st = document.createElement('style');
        st.textContent = '@keyframes reconnSpin{to{transform:rotate(360deg)}}';
        document.head.appendChild(st);
        document.body.appendChild(ov);
    }
    ov.style.display = 'flex';
    console.log('[Recovery] 리커넥트 오버레이 표시');
}

function hideReconnectOverlay() {
    const ov = document.getElementById('reconnectOverlay');
    if (ov) ov.style.display = 'none';
    if (_reconnectOverlayTimer) { clearTimeout(_reconnectOverlayTimer); _reconnectOverlayTimer = null; }
}

function updateReconnectMessage(msg, sub) {
    const el = document.getElementById('reconnMsg');
    const subEl = document.getElementById('reconnSub');
    if (el) el.textContent = msg;
    if (subEl) subEl.textContent = sub || '';
}

function startServerHealthPoll() {
    if (_serverHealthTimer) return;
    _serverIsDown = true;
    console.log('[Recovery] 서버 헬스체크 폴링 시작');
    _serverHealthTimer = setInterval(async () => {
        const alive = await checkServerHealth();
        if (alive && _serverIsDown) {
            console.log('[Recovery] 서버 복구 감지! 리로드...');
            stopServerHealthPoll();
            updateReconnectMessage('서버 복구 완료!', '새로고침 중...');
            setTimeout(() => location.reload(), 800);
        } else if (!alive) {
            updateReconnectMessage('서버에 연결하는 중...', '서버가 응답하지 않습니다');
        }
    }, 5000);
}

function stopServerHealthPoll() {
    if (_serverHealthTimer) { clearInterval(_serverHealthTimer); _serverHealthTimer = null; }
    _serverIsDown = false;
}
...
function startHeartbeatMonitor() {
    ...
    heartbeatTimer = setInterval(() => {
        ...
        if (elapsed > 20000) {
            ...
            // 15초 경과 - 경고 로그만
            console.log(`[WS] 하트비트: ${Math.round(elapsed/1000)}초 경과 (20초 후 재연결)`);
        }
    }, 3000);  // 3초마다 체크
}
...
function reconnectWithBackoff() {
    ...
    // ★★★ 재연결 3회 실패 + 20초 경과 + 서버 다운 확인 시에만 오버레이 ★★★
    if (reconnectAttempt >= 3 && !_reconnectOverlayTimer) {
        _reconnectOverlayTimer = setTimeout(async () => {
            if (!ws || ws.readyState !== WebSocket.OPEN) {
                const alive = await checkServerHealth();
                if (!alive) {
...
function connectWebSocket() {
    ...
    ws.onopen = function() {
        ...
        if (_wsHasConnectedBefore) {
            ...
            if (_prevReconnectAttempt >= 5 || window._serverWasDown) {
    ...
    ws.onmessage = function(event) {
        ...
        if (isDemo) {
            ...
            if (typeof QuickEasyPanel !== 'undefined' && data.positions && Array.isArray(data.positions)) {
                ...
                data.positions.filter(p => p.magic == 100003).forEach(qePos => {
                    ...
                    const _entry = qePos.entry || qePos.openPrice || 0;
                    const _vol = qePos.volume || 0;
                    // ★ target 복원 우선순위: 서버 → localStorage → TP/SL 역계산
                    let _target = qePos.target || 0;
                    if (!_target) _target = qeLoadTargetLS(posSym);
                    if (!_target && _tp > 0 && _sl > 0 && _entry > 0 && _vol > 0) {
                        _target = calcTargetFromTPSL(_entry, _tp, _sl, _vol, posSym);
                        if (_target > 0) console.log('[WS Demo] ★ target 역계산:', posSym, '$' + _target);
                    }
        ...
        // ★★★ Live 입출금 탭 실시간 업데이트 (라이브 WS) ★★★
        {
            ...
            if (myLiveEquity) myLiveEquity.textContent = fmtUSD(data._realtimeEquity || data.equity);
            const myLiveMargin = document.getElementById('myLiveMargin');
            if (myLiveMargin) myLiveMargin.textContent = fmtUSD(data.margin);
            const myLiveFree = document.getElementById('myLiveFreeMargin');
            if (myLiveFree) myLiveFree.textContent = fmtUSD(data.free_margin);
            const myLiveProfit = document.getElementById('myLiveProfit');
            if (myLiveProfit) {
                // 전체 포지션 손익 합산 (실시간 계산된 값 사용)
                let totalProfit = 0;
                if (data.positions && Array.isArray(data.positions)) {
                ...
                myLiveProfit.textContent = totalProfit === 0 ? '\$0.00' : (totalProfit >= 0 ? '+\$' : '-\$') + Math.abs(totalProfit).toLocaleString('en-US', { minimumFractionDigits: 2 });
                myLiveProfit.className = 'my-live-stat-value' + (totalProfit > 0 ? ' profit-plus' : totalProfit < 0 ? ' profit-minus' : '');
            }
        }
        ...
        if (typeof QuickEasyPanel !== 'undefined' && data.positions && Array.isArray(data.positions)) {
            ...
            data.positions.filter(p => p.magic == 100003).forEach(qePos => {
                ...
                if (!_target && _tp > 0 && _sl > 0 && _entry > 0 && _vol > 0) {
                    _target = calcTargetFromTPSL(_entry, _tp, _sl, _vol, posSym);
                    if (_target > 0) console.log('[WS Live] ★ target 역계산:', posSym, '$' + _target);
                }
                newQePositions[posSym] = {
                    id: qePos.id || qePos.ticket,
                    side: (qePos.type && qePos.type.includes('BUY')) ? 'BUY' : 'SELL',
            ...
            _reconnectOverlayTimer = setTimeout(async () => {
                if (!ws || ws.readyState !== WebSocket.OPEN) {
                    ...
                    if (!alive) {
                        ...
                        console.log('[Recovery] 서버 정상 — 오버레이 생략, WS만 재연결');
                    }
                }
            }, 15000);
        }
...
async function checkMetaAPIStatus() {
    try {
        ...
        if (status === 'deployed') {
            ...
            // ★ 에러는 콘솔 로그만 (반복 토스트 방지 — MT5 연결 섹션에서 확인 가능)
            console.warn('[MetaAPI] 상태 에러:', errorDetail);
            if (mt5StatusEl) {
